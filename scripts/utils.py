...
def dump_json(json_info, json_file, overwrite=True):
    ...
    if overwrite:
        ...
        mode = "a+"
    try:
        with open(json_file, mode) as outF:
            outF.write(json.dumps(json_info))
            outF.write("\n")
...
def parse_multiple_json(json_file, offset=None, maxLoadLine=None):
    ...
    try:
        ...
        with open(json_file, "r") as f:
            ...
            for line in f:
                ...
                offset += len(line)
                lnCnt += 1
                if maxLoadLine is not None and lnCnt >= maxLoadLine:
                    break
    except BaseException as e:
        logging.error(e.message)
